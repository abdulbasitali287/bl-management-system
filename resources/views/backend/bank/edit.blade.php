...
<div class="container-fluid">
    ...
    <div class="row">
        <div class="col-lg-6">
                {!! Form::open([
                    'url' => route('bank.update',$update->bid),
                    'method' => 'put'
                ]) !!}
                <div class="form-group my-3">
                    {!! Form::label("", "Bank Name", ["class" => "py-2"]) !!}
                    {!! Form::select("name", $banks, $selectedBank , ["class" => "form-control"]) !!}
                    @error("name")
                ...
                <div class="form-group my-3">
                    ...
                    {!! Form::text("branch", old('branch',$update->branch), ["class" => "form-control"]) !!}
                    @error("branch")
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
                </div>
                <div class="form-check form-switch my-3">
                    {!! Form::checkbox("status", true , $update->status == 1 ? "checked" : "" , ["class" => "form-check-input", "role" => "switch"  ]) !!}
